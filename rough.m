...
S1 = 0.098;    L_f1 = 0.2;           % Surface area of caudal fin
S2 = 0.044;    L_f2 = 0.1;
S3 = 0.044;    L_f3 = 0.1;
% Example data for lengths (l) and widths (b) and corresponding surface areas (S)
%% Example data for lengths (l) and widths (b) and corresponding surface areas (S)
l_data = [0.2, 0.1, 0.1]';   % Column vector (3x1)
b_data = [1,   2,   3]';       % Column vector (3x1)
S_data = [0.098, 0.044, 0.044]';  % Column vector (3x1)

% Build the design matrix for a linear relation: S = alpha*l + beta*b + gamma
X = [l_data, b_data, ones(length(l_data),1)]; 

% Compute the regression coefficients [alpha; beta; gamma]
coef = X \ S_data;  

% Now, compute S for each fin using the corresponding l and b values
...
for i=1:length(t)
    ...
    J =  [J1   null;
      ...
      Ux=U(1);Uy=U(2);Uz=U(3);
          [etad, etad_dot, etad_ddot] = generate_trajectory(t,'8', dt);
...
pause(0.01)
frame = getframe(gcf);
resizedFrame = imresize(frame.cdata, [420, 560]);  % Resize to 420 rows (height) x 560 columns (width)
